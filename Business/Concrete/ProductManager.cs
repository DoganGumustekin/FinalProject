...
namespace Business.Concrete
{
    ...
    {
        IProductDal _productDal; //constructor tanımladık bu diyorki bize productmanager newlwndiğinde bana productdal referansı ver.
                                 //bu referans bir entity framework olabilir. veri erişim tekniği değiştiğinde bir InMemory de olabilir.
                                 //bunu sağladık. 
        ...
        {
            var result = _productDal.GetAll(p => p.ProductId == categoryId).Count; //örneğin kategoryid=2 olan üründen kaç tane var??
            if (result >= 10)
            {
        ...
        private IResult CheckIfProductNameExists(string productName)
        {
            var result = _productDal.GetAll(p => p.ProductName == productName).Any(); //ürün isimlerinde benim gönerdiğim ürün ismi varmı?
                                                                                      //any varmı demek bu şarta uyan data varmı. 
            if (result) //result==true ile aynı. 
            {
